...
import ir.bank.qh.party.core.service.PersonEntityService;
...
public class PersonEntityController {
    ...
    private final PersonEntityService service;

    @GetMapping
    public ApiResponse<List<PersonEntity>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<PersonEntity> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<PersonEntity> create(@Valid @RequestBody PersonEntity entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<PersonEntity> update(@PathVariable Long id, @Valid @RequestBody PersonEntity entity) {
