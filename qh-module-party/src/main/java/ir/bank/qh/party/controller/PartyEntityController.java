...
/**
 * REST endpoints for PartyEntity. Every method is written out explicitly here
 * (no AbstractCrudController base), delegating to PartyEntityService.
...
public class PartyEntityController {
    ...
    public ApiResponse<PartyEntity> create(@Valid @RequestBody PartyEntity entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<PartyEntity> update(@PathVariable Long id, @Valid @RequestBody PartyEntity entity) {
