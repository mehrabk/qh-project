...
Usage: python3 generate_party_model.py
Reads:  qh-module-party/src/main/java/ir/bank/qh/party/{entity,controller}/*.java
...
ENTITY_DIR = os.path.join(REPO_ROOT, 'qh-module-party/src/main/java/ir/bank/qh/party/entity')
CONTROLLER_DIR = os.path.join(REPO_ROOT, 'qh-module-party/src/main/java/ir/bank/qh/party/controller')
...
def parse_controller(entity_name):
    ctrl_path = os.path.join(CONTROLLER_DIR, entity_name + 'Controller.java')
...
def main():
    ...
    entity_files = sorted(fn for fn in os.listdir(ENTITY_DIR) if fn.endswith('.java'))
    out_entities = []
    for fn in entity_files:
        meta, raw_fields = parse_entity(os.path.join(ENTITY_DIR, fn))
