...
public class ProductPricingRuleController {

    private final ProductPricingRuleService service;

    @GetMapping
    public ApiResponse<List<ProductPricingRule>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<ProductPricingRule> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<ProductPricingRule> create(@Valid @RequestBody ProductPricingRule entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<ProductPricingRule> update(@PathVariable Long id, @Valid @RequestBody ProductPricingRule entity) {
