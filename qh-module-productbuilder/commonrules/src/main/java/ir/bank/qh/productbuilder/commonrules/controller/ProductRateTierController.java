...
public class ProductRateTierController {

    private final ProductRateTierService service;

    @GetMapping
    public ApiResponse<List<ProductRateTier>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<ProductRateTier> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<ProductRateTier> create(@Valid @RequestBody ProductRateTier entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<ProductRateTier> update(@PathVariable Long id, @Valid @RequestBody ProductRateTier entity) {
