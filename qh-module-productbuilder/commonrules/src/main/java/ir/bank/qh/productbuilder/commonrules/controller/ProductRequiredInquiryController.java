...
public class ProductRequiredInquiryController {

    private final ProductRequiredInquiryService service;

    @GetMapping
    public ApiResponse<List<ProductRequiredInquiry>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<ProductRequiredInquiry> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<ProductRequiredInquiry> create(@Valid @RequestBody ProductRequiredInquiry entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<ProductRequiredInquiry> update(@PathVariable Long id, @Valid @RequestBody ProductRequiredInquiry entity) {
