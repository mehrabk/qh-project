...
import ir.bank.qh.productbuilder.core.service.ProductLegacyMappingService;
...
public class ProductLegacyMappingController {

    private final ProductLegacyMappingService service;

    @GetMapping
    public ApiResponse<List<ProductLegacyMapping>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<ProductLegacyMapping> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<ProductLegacyMapping> create(@Valid @RequestBody ProductLegacyMapping entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<ProductLegacyMapping> update(@PathVariable Long id, @Valid @RequestBody ProductLegacyMapping entity) {
