...
import ir.bank.qh.productbuilder.core.service.ProductVersionModuleService;
...
public class ProductVersionModuleController {

    private final ProductVersionModuleService service;

    @GetMapping
    public ApiResponse<List<ProductVersionModule>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<ProductVersionModule> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<ProductVersionModule> create(@Valid @RequestBody ProductVersionModule entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<ProductVersionModule> update(@PathVariable Long id, @Valid @RequestBody ProductVersionModule entity) {
