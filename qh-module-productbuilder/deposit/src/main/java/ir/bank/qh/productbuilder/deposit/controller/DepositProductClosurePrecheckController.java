...
public class DepositProductClosurePrecheckController {

    private final DepositProductClosurePrecheckService service;

    @GetMapping
    public ApiResponse<List<DepositProductClosurePrecheck>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<DepositProductClosurePrecheck> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<DepositProductClosurePrecheck> create(@Valid @RequestBody DepositProductClosurePrecheck entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<DepositProductClosurePrecheck> update(@PathVariable Long id, @Valid @RequestBody DepositProductClosurePrecheck entity) {
