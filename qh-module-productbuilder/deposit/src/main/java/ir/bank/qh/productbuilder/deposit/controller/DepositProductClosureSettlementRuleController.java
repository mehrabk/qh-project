...
public class DepositProductClosureSettlementRuleController {

    private final DepositProductClosureSettlementRuleService service;

    @GetMapping
    public ApiResponse<List<DepositProductClosureSettlementRule>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<DepositProductClosureSettlementRule> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<DepositProductClosureSettlementRule> create(@Valid @RequestBody DepositProductClosureSettlementRule entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<DepositProductClosureSettlementRule> update(@PathVariable Long id, @Valid @RequestBody DepositProductClosureSettlementRule entity) {
