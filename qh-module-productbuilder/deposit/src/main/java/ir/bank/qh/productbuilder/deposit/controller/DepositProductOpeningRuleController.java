...
public class DepositProductOpeningRuleController {

    private final DepositProductOpeningRuleService service;

    @GetMapping
    public ApiResponse<List<DepositProductOpeningRule>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<DepositProductOpeningRule> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<DepositProductOpeningRule> create(@Valid @RequestBody DepositProductOpeningRule entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<DepositProductOpeningRule> update(@PathVariable Long id, @Valid @RequestBody DepositProductOpeningRule entity) {
