...
import ir.bank.qh.productbuilder.deposit.service.DepositProductProfileService;
...
public class DepositProductProfileController {

    private final DepositProductProfileService service;

    @GetMapping
    public ApiResponse<List<DepositProductProfile>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<DepositProductProfile> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<DepositProductProfile> create(@Valid @RequestBody DepositProductProfile entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<DepositProductProfile> update(@PathVariable Long id, @Valid @RequestBody DepositProductProfile entity) {
