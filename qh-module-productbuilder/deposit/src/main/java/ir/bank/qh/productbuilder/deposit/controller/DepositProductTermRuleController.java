...
public class DepositProductTermRuleController {

    private final DepositProductTermRuleService service;

    @GetMapping
    public ApiResponse<List<DepositProductTermRule>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<DepositProductTermRule> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<DepositProductTermRule> create(@Valid @RequestBody DepositProductTermRule entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<DepositProductTermRule> update(@PathVariable Long id, @Valid @RequestBody DepositProductTermRule entity) {
