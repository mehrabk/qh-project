...
public class LoanProductProcessRuleController {

    private final LoanProductProcessRuleService service;

    @GetMapping
    public ApiResponse<List<LoanProductProcessRule>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<LoanProductProcessRule> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<LoanProductProcessRule> create(@Valid @RequestBody LoanProductProcessRule entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<LoanProductProcessRule> update(@PathVariable Long id, @Valid @RequestBody LoanProductProcessRule entity) {
