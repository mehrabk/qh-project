...
import ir.bank.qh.productbuilder.loan.service.LoanProductProfileService;
...
public class LoanProductProfileController {

    private final LoanProductProfileService service;

    @GetMapping
    public ApiResponse<List<LoanProductProfile>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<LoanProductProfile> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<LoanProductProfile> create(@Valid @RequestBody LoanProductProfile entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<LoanProductProfile> update(@PathVariable Long id, @Valid @RequestBody LoanProductProfile entity) {
