...
public class EconomicSubSectionController {

    private final EconomicSubSectionService service;

    @GetMapping
    public ApiResponse<List<EconomicSubSection>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<EconomicSubSection> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<EconomicSubSection> create(@Valid @RequestBody EconomicSubSection entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<EconomicSubSection> update(@PathVariable Long id, @Valid @RequestBody EconomicSubSection entity) {
