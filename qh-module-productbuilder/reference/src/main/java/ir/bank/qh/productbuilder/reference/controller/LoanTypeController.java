...
public class LoanTypeController {
    ...
    @GetMapping
    public ApiResponse<List<LoanType>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<LoanType> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<LoanType> create(@Valid @RequestBody LoanType entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<LoanType> update(@PathVariable Long id, @Valid @RequestBody LoanType entity) {
