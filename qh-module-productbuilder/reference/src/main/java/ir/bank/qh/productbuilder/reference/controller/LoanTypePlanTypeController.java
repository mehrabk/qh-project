...
public class LoanTypePlanTypeController {

    private final LoanTypePlanTypeService service;

    @GetMapping
    public ApiResponse<List<LoanTypePlanType>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<LoanTypePlanType> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<LoanTypePlanType> create(@Valid @RequestBody LoanTypePlanType entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<LoanTypePlanType> update(@PathVariable Long id, @Valid @RequestBody LoanTypePlanType entity) {
