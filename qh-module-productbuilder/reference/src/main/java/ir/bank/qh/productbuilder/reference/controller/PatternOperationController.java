...
public class PatternOperationController {

    private final PatternOperationService service;

    @GetMapping
    public ApiResponse<List<PatternOperation>> findAll() {
        return ApiResponse.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<PatternOperation> findById(@PathVariable Long id) {
        return ApiResponse.ok(service.findById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<PatternOperation> create(@Valid @RequestBody PatternOperation entity) {
        return ApiResponse.ok(service.create(entity), "با موفقیت ایجاد شد");
    }

    @PutMapping("/{id}")
    public ApiResponse<PatternOperation> update(@PathVariable Long id, @Valid @RequestBody PatternOperation entity) {
